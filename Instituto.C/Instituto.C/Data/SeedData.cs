...
namespace Instituto.C.Data
{
    public static class SeedData
    {
        public static async Task InitializeAsync(InstitutoDb context, UserManager<Persona> userManager)
        {
            ...
            if (carreraAnalista == null || carreraCiencias == null)
                throw new Exception("Error al crear o recuperar las carreras base.");
            ...
            {
                string[] nombresTecnologia = new[] { "Ada", "Alan", "Grace", "Linus", "Dennis", "Ken", "Margaret", "Tim", "Elon", "Bill", "Steve", "Mark", "Guido", "James", "Donald" };
                string[] apellidosTecnologia = new[] { "Lovelace", "Turing", "Hopper", "Torvalds", "Ritchie", "Thompson", "Hamilton", "Berners-Lee", "Musk", "Gates", "Jobs", "Zuckerberg", "van Rossum", "Gosling", "Knuth" };
                string[] nombresCiencias = new[] { "Marie", "Charles", "Isaac", "Rosalind", "Stephen", "Jane", "Gregor", "Carl", "Rachel", "Alexander", "Barbara", "Neil", "Lise", "Galileo", "Alfred" };
                string[] apellidosCiencias = new[] { "Curie", "Darwin", "Newton", "Franklin", "Hawking", "Goodall", "Mendel", "Sagan", "Carson", "Fleming", "McClintock", "Tyson", "Meitner", "Galilei", "Wallace" };
            ...
            // === MATERIAS CURSADAS ===
            var codigosAnalista = new[] { "OE101", "INF101", "FP101", "THP101", "MAT101", "ING101", "TCI101", "SA201", "ASO201", "P1", "TP1", "PP1", "BD1" };
            var materiasAnalista = await context.Materias.Where(m => codigosAnalista.Contains(m.CodigoMateria)).ToListAsync();
            var profesoresDisponibles = await context.Profesores.ToListAsync();

            if (!materiasAnalista.Any() || !profesoresDisponibles.Any())
                throw new Exception("No hay materias o profesores disponibles para crear materias cursadas.");
            ...
            {
                bool yaTieneCursada = await context.MateriasCursadas.AnyAsync(mc =>
                    mc.MateriaId == materia.Id &&
                    mc.Anio == anioActual &&
            ...
            if (!alumnos.Any() || !materiasCursadas.Any())
                throw new Exception("No hay alumnos o materias cursadas disponibles para generar inscripciones.");
